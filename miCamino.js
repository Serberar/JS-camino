...
//Habiendo visto como acceder a las propiedades de los distintos objetos,como podemos recorrer arrays y ir buceando en ellos, es el momento de 
//ponernos a prueba.
...
// primer paso: vamos a empezar nuestro camino de la mejor manera y esa no puede ser de otra manera que dando el primer paso, por tanto vamos a decir: 
//Lo mas importante es {inicio}
...
//4º: en este paso ya empezaremos a toquetear con bucles y le vamos a decir: lo primero que debo llevar es {nombre del primer material} y 
//es { su descripcion}
...
for (let index = 0; index < miCamino.material.length; index++) {
  const element = miCamino.material[index];
  ...
  if (index === 0) {
    ...
    console.log(`Y por último debo llevar la ${element.nombre} que es ${element.descripcion}.`);
  }
}
...
for (let index = 0; index < miCamino.cimas.length; index++) {
  const datos = miCamino.cimas[index];
 console.log(`para subir al ${datos.nombre}  debo subir ${datos.altitud} metros y debo viajar a ${datos.localizacion}`)
}
//6º: para llegar a la cima tendremos que darnos aliento y decirnos la frase que esta divida y que tiene que quedar así: 
//"Tranquilo,lo peor ha pasado ya.¡Animo!

let caminito = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV
let textoCampoIV = [];
for (let index = 0; index < caminito.length; index++) {
  if (typeof caminito[index] === "string") {
    textoCampoIV.push(caminito[index]);
  }
}
console.log(textoCampoIV.join(" "));

//7º: mostraremos el mensaje final guardado en la cima  
let llegue = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV[8].cima;
let textoCima= []

for (let index = 0; index < llegue.length; index++) {
  if (typeof llegue[index] === "string") {
    textoCima.push(llegue[index]); 
  }
